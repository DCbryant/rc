...
const flatten = (
  ...
  const arr: itemPropsRequired[] = []; //收集所有子项
  list.forEach(item => {
    item.level = level
...
const checkParent = (g: itemPropsRequired) => {
  return g.level === 1
}
...
const findOrigin = (key: string, data: itemPropsRequired[]) => {
...
export function Tree(props: TreeProps) {
	...
	const data = useMemo(() => {
		return flatten(root.children, 1, root, true);
